    ...
    public class MultiOpenSearchRequest<TFeed, TItem> : OpenSearchRequest where TFeed : class, IOpenSearchResultCollection, ICloneable, new() where TItem : class, IOpenSearchResultItem
    {
        ...
        public MultiOpenSearchRequest(OpenSearchableFactorySettings settings, IOpenSearchable[] entities, string type, OpenSearchUrl url, bool concurrent, IOpenSearchable parent) : base(url, type)
        {
            ...
            lock (_m2)
            {
                ...
                GetClosestState(entities.ToArray(), type, this.targetParameters, out this.currentEntities, out this.currentParameters);
            }

            var sc = new MethodSortingItemComparer();
			// By default sort on the LastUpdatedTime
            sc.SortMethods.Add((x, y) => -(x.LastUpdatedTime.CompareTo(y.LastUpdatedTime)));
            SortingComparer = sc;
        ...
        private void ExecuteConcurrentRequest()
        {
            ...
            Parallel.ForEach<IOpenSearchable>(currentEntities.Keys,
                                              ...
                                              (entity, pls) =>
                                              {
                                                  ...
                                                  Task task = Task.Factory.StartNew(() => ExecuteOneRequest(entity, pls), token);
                                                  task.Wait();
        ...
        void ExecuteOneRequest(object entity, ParallelLoopState pls)
        {
        ...
        TFeed Merge(TFeed f1, TFeed f2, IOpenSearchable os)
        {
            ...
            int common = f1.Items.Intersect(f2.Items, new OpenSearchResultItemComparer()).OrderBy(i => i, SortingComparer).Count();
            tfeed.Items = f1.Items.Union(f2.Items, new OpenSearchResultItemComparer()).OrderBy(i => i, SortingComparer);
            totalResults -= common;
