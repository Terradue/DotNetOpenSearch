...
namespace Terradue.OpenSearch.Test {
    ...
    public class ParametersTest : IClassFixture<TestFixture> {
        ...
        [Fact(DisplayName = "Template Test #1")]
        [Trait("Category", "unit")]
        public void TestTemplate() {
            ...
            Assert.Equal("bbox=10,10,20,20&bbox=30,30,40,40&start=2010-01-01&end=2011-01-01&test=&format=atom&other=value&notfound=bla", vs1.GetQueryString(true));
            Assert.Equal("bbox=10,10,20,20&bbox=30,30,40,40&start=2010-01-01&end=2011-01-01&format=atom&other=value&notfound=bla", vs1.GetQueryString(false));

            // Translate value set by identifier matching
            vs2.TranslateFrom(vs1);
            Assert.Equal("rechteck=10,10,20,20&rechteck=30,30,40,40&anfang=2010-01-01&ende=2011-01-01&format=json", vs2.GetQueryString(true));
            Assert.Equal("rechteck=10,10,20,20&rechteck=30,30,40,40&anfang=2010-01-01&ende=2011-01-01&format=json", vs2.GetQueryString(false));
            ...
            // Verify search query string correctness
            Assert.Equal("bbox=1,1,2,2&bbox=3,3,4,4&start=2012-01-01&end=2013-01-01&test=&format=atom&other=value", vs3.GetQueryString(true));
            Assert.Equal("bbox=1,1,2,2&bbox=3,3,4,4&start=2012-01-01&end=2013-01-01&format=atom&other=value", vs3.GetQueryString(false));
        }

        //---------------------------------------------------------------------------------------------------------------------

        [Fact(DisplayName = "Deserialize Objects Test")]
        [Trait("Category", "unit")]
        public void TestDeserializedObjects() {
            ...
            OpenSearchDescription osd1 = (OpenSearchDescription)ser.Deserialize(XmlReader.Create(TestFixture.TestBaseDir + "/Samples/ParametersTest1.osd.xml"));
            ...
            OpenSearchDescription osd2 = (OpenSearchDescription)ser.Deserialize(XmlReader.Create(TestFixture.TestBaseDir + "/Samples/ParametersTest2.osd.xml"));
            ...
            OpenSearchDescription osd3 = (OpenSearchDescription)ser.Deserialize(XmlReader.Create(TestFixture.TestBaseDir + "/Samples/ParametersTest3.osd.xml"));
            ...
            // Verify search query string correctness
            Assert.Equal("format=atom&count=&startPage=&startIndex=&q=test search&start=&stop=&trel=&bbox=5,5,6,6&uid=", vs1.GetQueryString(true));
            Assert.Equal("format=atom&q=test search&bbox=5,5,6,6", vs1.GetQueryString(false));

            // Translate value set by basic identifier matching
            vs2.TranslateFrom(vs1);
            Assert.Equal("format=atom&count2=&startPage2=&startIndex2=&q2=test search&start2=&stop2=&trel2=&bbox2=5,5,6,6&uid2=", vs2.GetQueryString(true));
            Assert.Equal("format=atom&q2=test search&bbox2=5,5,6,6", vs2.GetQueryString(false));

            // Translate value set by advanced identifier matching (verify namespace URIs)
            vs3.TranslateFrom(vs1, true);
            Assert.Equal("format=atom&count3=&startPage3=&startIndex3=&q3=test search&start3=&stop3=&trel3=&bbox3=5,5,6,6&uid3=", vs3.GetQueryString(true));
            Assert.Equal("format=atom&q3=test search&bbox3=5,5,6,6", vs3.GetQueryString(false));
